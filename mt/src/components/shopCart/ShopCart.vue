<template>
  <div class="shopcart">
    <div class="shopcart-wrapper">
      ...
      <div class="content-left">
        <div class="logo-wrapper" :class="{highlight:totalCount}">
          <span class="icon-shopping_cart logo" :class="{highlight:totalCount}" @click="toggleList"></span>
          <i class="num" v-show="totalCount">{{totalCount}}</i>
        </div>
        <div class="desc-wrapper">
          <p class="total-price" v-show="totalPrice">￥{{totalPrice}}</p>
          <p class="tip" :class="{highlight:totalCount}">另需{{poiInfo.shipping_fee_tip}}</p>
        </div>
      </div>
      <!-- 底部右侧 -->
      <div class="content-right" :class="{highlight:totalCount}">{{payStr}}</div>

      <!-- 购物车详单 -->
      <transition name="shopcart-list">
      <div class="shopcart-list" v-show="isShow" :class="{'show':isShow}">
    ...
    <div class="shopcart-mask" v-show="isShow" @click="hideMask"></div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    totalPrice() {
      let totalPrice = 0;
      this.cartList.forEach(food => {
        totalPrice += food.min_price * food.count;
      });
      return totalPrice;
    },
    payStr() {
  ...
  methods: {
    toggleList() {
      ...
      this.show_flag_1 = !this.show_flag_1;
    },
    clearAll() {
      this.cartList.forEach(food => {
        food.count = 0;
  ...
  //       if (val) {
  //         this.$nextTick(() => {
  //           if (Object.keys(this.shopScroll).length === 0) {
  //             this.shopScroll = new BScroll(this.$refs.listContent, {
  //               click: true
  //             });
  //           } else {
