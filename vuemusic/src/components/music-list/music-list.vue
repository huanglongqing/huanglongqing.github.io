...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    mounted(){
      ...
      this.maxTranslateY = -this.imgHeight + RESERVED_HEIGHT;
    ...
    watch:{
      scrollY(nVal){
        let translateY = Math.max(this.maxTranslateY, nVal);
        let scale = 1;
        let zIndex = 0;
        let blur = 0
        const percent = Math.abs(nVal / this.imgHeight);
        if (nVal > 0) {
          scale = 1 + percent;
          zIndex = 10;
        } else {
...
<style scoped lang="stylus" rel="stylesheet/stylus">
  ...
  .music-list
    ...
    .bg-image
      ...
      height: 0
      padding-top: 70%
      transform-origin: top            //transform从顶变化
